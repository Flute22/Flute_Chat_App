...
import { ADD_PROFILE_IMAGE, Delete_PROFILE_IMAGE, HOST, UPDATE_PROFILE_INFO } from "../../utils/constants";
...
function Profile() {
  ...
  const handleImageChange = async (event) => {
    ...
    if (file) {
      ...
      const response = await apiClient.put(ADD_PROFILE_IMAGE, formData, {
        withCredentials: true,
      });
      console.log(response);
  ...
  return (
    <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10">
      <div className="flex flex-col gap-10 w-[80vw] md:w-max">
        <div className="">
          <IoArrowBack
            onClick={handleNavigationArrow}
            className="text-3xl lg:text-3xl text-white opacity-50 hover:opacity-90 cursor-pointer"
          />
        </div>
        <div className="grid grid-cols-2">
          ...
          >
            <Avatar
              className={`h-20 w-20 md:w-48 md:h-48 flex items-center justify-center rounded-full overflow-hidden ${getColor(
                selectedColor
              )}`}
            >
              {image ? (
            ...
            {hovered && (
              <div
                className="absolute w-20 h-20 md:w-48 md:h-48 flex items-center justify-center bg-black/50 rounded-full cursor-pointer"
                onClick={image ? handleDeleteImage : handleFileInputClick}
              >
                {image ? (
                  <FaTrash className="text-gray-300 text-xl" />
                ) : (
